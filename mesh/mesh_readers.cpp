...
void CubitInfo(std::string var_name, std::string var_val){
    std::cout << "\x1b[34m" << "Cubit .g   " << "\x1b[0m" << var_name << " " << var_val << std::endl;
}
...
void Mesh::ReadCubit(const char *filename, int &curved, int &read_gf)
{
   ...
   CubitInfo(toStr(num_dim),std::to_string(num_dim));
    CubitInfo(toStr(num_nodes),std::to_string(num_nodes));
    CubitInfo(toStr(num_elem),std::to_string(num_elem));
    CubitInfo(toStr(num_el_blk),std::to_string(num_el_blk));
    CubitInfo(toStr(num_side_sets),std::to_string(num_side_sets));
   ...
   for (int i = 0; i < (int) num_el_blk; i++)
   {
      ...
      CubitInfo(temp_str,std::to_string(num_el_in_blk[i]));
      ...
      CubitInfo(temp_str,std::to_string(num_node_per_el));
   ...
   else if (num_dim == 3) // 3D
   {
      switch (num_node_per_el)
      {
         case (4) : // tetra element
         {
   ...
   CubitInfo(toStr(order),std::to_string(order));
   ...
   for (int i = 0; i < (int) num_side_sets; i++)
   {
      ...
      CubitInfo(temp_str,std::to_string(num_side_in_ss[i]));
   }
   ...
   CubitInfo(toStr(NumOfVertices),std::to_string(NumOfVertices));

   // scale coord
   for (int i = 0; i < (int) uniqueVertexID.size(); i++)
   {
      coordx[uniqueVertexID[i] - 1] *= 0.001;
      coordy[uniqueVertexID[i] - 1] *= 0.001;
      if (Dim == 3)
      {
         coordz[uniqueVertexID[i] - 1] *= 0.001;
      }
   }
   ...
   CubitInfo(toStr(NumOfElements),std::to_string(NumOfElements));
   ...
   CubitInfo(toStr(NumOfBdrElements),std::to_string(NumOfBdrElements));
   boundary.SetSize(NumOfBdrElements);
   int sidecount = 0;
   for (int iss = 0; iss < (int) num_side_sets; iss++)
   {
      CubitInfo(toStr(ssprop[iss]),std::to_string(ssprop[iss]));
