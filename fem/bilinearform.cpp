...
void BilinearForm::AssembleElementMatrix(int i, const DenseMatrix &elmat, Array<int> &vdofs, int skip_zeros)
{
   fes->GetElementVDofs(i, vdofs);
   if (static_cond)
   {
...
void BilinearForm::Assemble(int skip_zeros)
{
   ...
   if (domain_integs.Size())
   {
      ...
      for (int i = 0; i < fes -> GetNE(); i++) // go through the elements
      {
         ...
         {
            elmat.SetSize(0);
            for (int k = 0; k < domain_integs.Size(); k++) // go through the integrators
            {
               if ( domain_integs_marker[k] == NULL ||
